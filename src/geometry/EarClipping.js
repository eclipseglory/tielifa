    ...
    static isEar(ear) {
        ...
        while (p !== ear.prev) {
            if (this.pointInTriangle(a.x, a.y, b.x, b.y, c.x, c.y, p.x, p.y) &&
                EarClipping.area(p.prev, p, p.next) >= 0) return false;
            p = p.next;
        }
    ...
    static isEarHashed(ear, minX, minY, invSize) {
        ...
        while (p && p.z >= minZ && n && n.z <= maxZ) {
            ...
            if (n !== ear.prev && n !== ear.next &&
                ...
                EarClipping.area(n.prev, n, n.next) >= 0) return false;
            n = n.nextZ;
        }
...
// go through all polygon nodes and cure small local self-intersections
    static cureLocalIntersections(start, triangles, dim) {
        ...
        do {
            ...
            if (!EarClipping.equals(a, b) && this.intersects(a, p, p.next, b)
...
// check if two segments intersect
    static intersects(p1, q1, p2, q2) {
        if ((EarClipping.equals(p1, q1) && EarClipping.equals(p2, q2)) ||
            (EarClipping.equals(p1, q2) && EarClipping.equals(p2, q1))) return true;
        return EarClipping.area(p1, q1, p2) > 0 !== EarClipping.area(p1, q1, q2) > 0 &&
            EarClipping.area(p2, q2, p1) > 0 !== EarClipping.area(p2, q2, q1) > 0;
    }
...
// check if a polygon diagonal is locally inside the polygon
    static locallyInside(a, b) {
        return EarClipping.area(a.prev, a, a.next) < 0 ?
            EarClipping.area(a, b, a.next) >= 0 && EarClipping.area(a, a.prev, b) >= 0 :
            EarClipping.area(a, b, a.prev) < 0 || EarClipping.area(a, a.next, b) < 0;
    }
